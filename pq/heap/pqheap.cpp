...
{    
    ...
    void PQHeap<Data>::RemoveTip() noexcept(false)
    {
        ...
        std::cout << "\n\nsize of heapVec (before resize): " << this->heapVec.Size();
        std::cout << "\nsize of pq (before resize): " << this->Size() << "\n\n";

        if(this->size == this->Capacity()/2)
            this->Resize(this->size);

        std::cout << "\n\nsize of heapVec (after resize, before decreasing): " << this->heapVec.Size();
        std::cout << "\nsize of pq (after resize, before decreasing): " << this->Size() << "\n\n";
        
        this->heapVec[0] = this->heapVec[this->size-1];
        this->size--;
       // this->heapVec.DecreaseHeapSize(this->size);

        std::cout << "\n\nsize of heapVec (after decreasing): " << this->heapVec.Size();
        std::cout << "\nsize of pq (after decreasing): " << this->Size() << "\n\n";
...
/*         while (lastIndex > 0) 
        {
            if( this->heapVec[lastIndex] > this->heapVec[this->heapVec.GetParentIndex(lastIndex)])
            {
                std::swap(this->heapVec[lastIndex], this->heapVec[this->heapVec.GetParentIndex(lastIndex)]);
                lastIndex = this->heapVec.GetParentIndex(lastIndex);
            }
            else
                break;
        } */
        
/*         while(lastIndex > 0 && this->heapVec[lastIndex] > this->heapVec[this->heapVec.GetParentIndex(lastIndex)])
        {
            std::cout << "\n\nciclo while...";

            std::cout << "\nlastIndex prima swap: " << lastIndex;

            std::swap(this->heapVec[lastIndex], this->heapVec[this->heapVec.GetParentIndex(lastIndex)]);
            lastIndex = this->heapVec.GetParentIndex(lastIndex);

            std::cout << "\nlastIndex dopo swap: " << lastIndex;
...
/*         {
            while (index > 0) 
            {
                if( this->heapVec[index] > this->heapVec[this->heapVec.GetParentIndex(index)])
                {
                    std::swap(this->heapVec[index], this->heapVec[this->heapVec.GetParentIndex(index)]);
                    index = this->heapVec.GetParentIndex(index);
                }
                else
                    break;
            } */
/*             while(index > 0 && this->heapVec[index] > this->heapVec[this->heapVec.GetParentIndex(index)])
            {
                std::swap(this->heapVec[index], this->heapVec[this->heapVec.GetParentIndex(index)]);
                index = this->heapVec.GetParentIndex(index);
            } */
