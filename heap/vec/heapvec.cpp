...
namespace lasd 
{
    ...
    HeapVec<Data>::HeapVec(const ulong size) : Vector<Data>::Vector(size)
    {
        this->Heapify();
    }
    ...
    template<typename Data>
    HeapVec<Data>::HeapVec(const TraversableContainer<Data>& traversableContainer) : Vector<Data>::Vector(traversableContainer)
    {
        this->Heapify();
    }
    ...
    template<typename Data>
    HeapVec<Data>::HeapVec(MappableContainer<Data>&& mappableContainer) : Vector<Data>::Vector(std::move(mappableContainer))
    {
        this->Heapify();
    }
    ...
    template<typename Data>
    HeapVec<Data>::HeapVec(const HeapVec<Data>& heapVec) : Vector<Data>::Vector(heapVec)
    {}


    template<typename Data>
    HeapVec<Data>::HeapVec(HeapVec<Data>&& heapVec) noexcept : Vector<Data>::Vector(std::move(heapVec))
    {}
...
namespace lasd 
{
    ...
    {
        this->size = this->sortableVector.Size();
        this->Heapify();
    }
    ...
    HeapVec<Data>::HeapVec(const HeapVec<Data>& heapVec) : sortableVector(heapVec.size)
    {
        ...
        for (ulong i = 0; i < this->size; ++i)
            this->sortableVector[i] = heapVec[i];
    }
    ...
    bool HeapVec<Data>::operator==(const HeapVec<Data>& heapVec) const noexcept
    {
        return LinearContainer<Data>::operator==(heapVec);
    }
    ...
    bool HeapVec<Data>::IsHeap() const
    {
        ...
        {
            ulong parentIndex = GetParentIndex(index);
            if(this->sortableVector[index] > this->sortableVector[parentIndex])
                return false;
        }
    ...
    void HeapVec<Data>::Sort()
    {
        ...
        for(ulong index = (this->size)-1; index > 0; index--)
        {
            std::swap(this->sortableVector[0], this->sortableVector[index]);
            this->size--;
            this->HeapifyDown(0);
        }
    ...
    void HeapVec<Data>::HeapifyDown(ulong index)
    {
        ...
        if(max != index)
        {
            std::swap(this->sortableVector[index], this->sortableVector[max]);
            HeapifyDown(max);
        }
    }
} */
